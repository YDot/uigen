...
test("renders chat interface with empty state when no messages", () => {
  ...
  // When there are no messages, the empty state UI is shown instead of MessageList
...
test("scrolls when messages change", () => {
  // Start with messages so the MessageList is rendered
  (useChat as any).mockReturnValue({
    ...mockUseChat,
    messages: [{ id: "1", role: "user", content: "Hello" }],
  });
...
test("renders with correct layout classes", () => {
  // Render with messages to test the MessageList path
  (useChat as any).mockReturnValue({
    ...mockUseChat,
    messages: [{ id: "1", role: "user", content: "Hello" }],
  });
